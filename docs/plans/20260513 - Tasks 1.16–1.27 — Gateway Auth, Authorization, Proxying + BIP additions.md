...
app:
  ...
  oauth:
    google:
      ...
      token-uri: https://oauth2.googleapis.com/token               # ← added in impl
      userinfo-uri: https://openidconnect.googleapis.com/v1/userinfo  # ← added in impl
    github:
      client-id: ${GITHUB_CLIENT_ID}
      client-secret: ${GITHUB_CLIENT_SECRET}
      redirect-uri: ${APP_BASE_URL}/v1/auth/oauth/github/callback
      token-uri: https://github.com/login/oauth/access_token       # ← added in impl
      userinfo-uri: https://api.github.com/user                     # ← added in impl
  rate-limit:
    enabled: ${RATE_LIMIT_ENABLED:true}                            # ← added in impl
...
app:
  ...
  oauth:
    ...
    github:
      ...
      userinfo-uri: https://api.github.com/user                  # ← added in impl
  rate-limit:
    enabled: true              # ← added in impl; set false in CI to skip Redis
...
> **Security note:** `application-dev.yml` MUST contain only dummy/placeholder credentials (`dev-google-client-id`, fake API keys, `test-mode: true`). Real OAuth secrets and Resend API keys belong in a `.env` file or secret manager and must NEVER be committed. The committed `application-dev.yml` initially contained real credentials — these must be rotated and replaced with placeholders.
...
│   └── dto/
│       ├── ApiResponse.java
│       ├── RegisterRequest.java               (email, password, orgName?)
│       ├── RegisterResponse.java              (tenantId) ← NOT TokenResponse; register does not issue tokens
...
├── application/
│   ├── RegisterUserUseCase.java               (interface — returns RegisterResponse)
...
│   └── impl/
│       ├── RegisterUserUseCaseImpl.java        (creates Tenant + User + sends OTP async)
...
├── domain/
│   ├── Tenant.java                            (record — id, tenantId, name, slug, plan)
...
│       ├── InvalidOAuthStateException.java
│       ├── InvalidOtpException.java
│       ├── UnauthorizedException.java
│       └── UnverifiedEmailException.java
├── infrastructure/
│   ├── jdbc/
│   │   ├── TenantRepository.java              (interface)
│   │   ├── UserRepository.java                (interface)
│   │   ├── RefreshTokenRepository.java        (interface — includes deleteExpiredAndRevoked)
...
│   │   ├── OAuthProfile.java                  (record — subject, email, name)
...
│   ├── scheduler/
│   │   └── RefreshTokenPurgeScheduler.java    (@Scheduled daily purge of expired/revoked tokens)
│   ├── security/
│   │   ├── JwtAuthentication.java             (AbstractAuthenticationToken holding JwtClaims)
│   │   ├── JwtAuthenticationWebFilter.java    (WebFilter — cookie or Bearer extraction)
│   │   ├── JsonAuthenticationEntryPoint.java  (returns error envelope on 401)
│   │   ├── JsonAccessDeniedHandler.java       (returns error envelope on 403)
│   │   ├── TenantInjectionGlobalFilter.java   (GlobalFilter — strips X-Tenant-Id, injects from JWT)
│   │   └── RateLimitWebFilter.java            (WebFilter — custom Lua token bucket, 429 on breach)
│   └── tracing/
│       └── TraceContext.java                  (extracts Micrometer tracing span ID from Reactor context)
└── config/
    ├── AppConfig.java                         (JwtEncoder/Decoder, OkHttpClient, WebClient.Builder beans; @EnableScheduling)
    ├── SecurityConfig.java                    (SecurityWebFilterChain; injects JwtTokenProvider + TraceContext)
    ├── JwtConfig.java                         (@ConfigurationProperties for jwt.*)
    ├── ResendConfig.java                      (@ConfigurationProperties for resend.*)
    ├── OAuthConfig.java                       (@ConfigurationProperties for oauth.* incl. token-uri + userinfo-uri)
    ├── RateLimitConfig.java                   (rateLimitScript + reactiveStringRedisTemplate beans)
    └── RateLimitProperties.java               (@ConfigurationProperties for rate-limit.* incl. enabled flag)
    ← GatewayRoutesConfig.java NOT created; routes live in application.yml
```
...
### `domain/Tenant.java` ← new; not in original plan
...
public record Tenant(
    ...
    UUID tenantId,
    String name,
    String slug,        // nullable — auto-derived from name
    String plan,        // "free" default
    Instant createdAt,
    Instant updatedAt,
...
Registration creates a new `Tenant` via `TenantRepository.save()` before creating the user. The `RegisterRequest` carries an optional `orgName` field; if blank, `email` is used as the tenant name.
...
```java
// Returns RegisterResponse(tenantId) — NOT TokenResponse.
...
Constructor injects `JwtTokenProvider` and `TraceContext` (not the filter beans directly). `JwtAuthenticationWebFilter` is defined as a `@Bean` inside `SecurityConfig` so that `JsonAuthenticationEntryPoint` and `JsonAccessDeniedHandler` can receive `TraceContext` from the same config class.
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        return http
            ...
            .authorizeExchange(ex -> ex
                .pathMatchers("/v1/auth/userinfo").authenticated()    // ← explicit before permitAll
...
> **Updated to reflect actual implementation — differs significantly from original plan.**

The implementation does **not** use Spring Cloud Gateway's built-in `RedisRateLimiter`. Instead, a custom `RateLimitWebFilter` (implements `WebFilter`, `@Order(HIGHEST_PRECEDENCE + 1)`) runs a hand-written Lua token bucket script against Redis via `ReactiveStringRedisTemplate`. This gives full control over the key structure, error handling, and 429 body.

### Why custom instead of SCG built-in

SCG's `RedisRateLimiter` only works on routed requests and cannot be applied to in-process WebFlux controllers (`/v1/auth/**`) without a `forward://` route hack. A `WebFilter` runs for ALL requests including direct controller calls.

### `config/RateLimitConfig.java`

Beans produced: `RedisScript<String> rateLimitScript` (the Lua token bucket) and `ReactiveStringRedisTemplate` (conditional on missing bean for test overrides).
...
public class RateLimitConfig {
    ...
    public RedisScript<String> rateLimitScript() {
        ...
        script.setScriptText(TOKEN_BUCKET_LUA);   // inline Lua string
...
### `infrastructure/security/RateLimitWebFilter.java`

- `@Order(Ordered.HIGHEST_PRECEDENCE + 1)` — runs before routing; after `GlobalTracingFilter`
- Bucket key: `rate_limit:auth:<clientIp>` for `/v1/auth/**`; `rate_limit:default:<clientIp>` for all other paths
- Keys: `{bucket}.tokens` and `{bucket}.timestamp` — Lua script does atomic token replenishment + decrement
- On Redis error: fail-open (`onErrorReturn(true)`) — rate limiting never blocks service on Redis outage
- `enabled` flag: `RateLimitProperties.enabled()` — set `RATE_LIMIT_ENABLED=false` in CI / tests to skip Redis
...
public record RateLimitProperties(
    boolean enabled,              // ← new vs. original plan
...
### `application.yml` — no rate-limit YAML routes needed

Route YAML contains only the registry proxy route. Rate limiting is handled entirely by `RateLimitWebFilter`.
...
429 responses are written directly by `RateLimitWebFilter.write429()` using `ApiErrorResponse` — no custom `ErrorWebExceptionHandler` needed.
...
> **Updated: placeholder numbers resolved to actual ADR numbers.**

### ADR-0010 — Gateway as sole token issuer

**File:** `docs/adr/ADR-0010-gateway-sole-token-issuer.md`
...
**File:** `docs/adr/ADR-0011-httponlycookie-bearer-dual-auth.md`
...
| `services/gateway/build.gradle.kts` | Modified — add security, JDBC, Redis, Caffeine deps (NO Flyway) |
| `services/gateway/src/main/resources/application.yml` | Modified — env vars only; includes `token-uri`/`userinfo-uri` per OAuth provider; `rate-limit.enabled` flag |
| `services/gateway/src/main/resources/application-dev.yml` | New — ⚠️ must use dummy credentials only; `test-mode: true` |
...
| `services/gateway/src/main/java/io/cartogra/gateway/api/dto/*.java` (8 records incl. `RegisterResponse`) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/domain/Tenant.java` | New ← not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/domain/User.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/domain/RefreshToken.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/domain/TenantOidcConfig.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/domain/exception/*.java` (5 files) | New ← package not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/application/*.java` (7 interfaces) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/application/impl/*.java` (7 impls) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/jdbc/TenantRepository.java` | New ← not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/jdbc/JdbcTenantRepository.java` | New ← not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/jdbc/*.java` (remaining 6 files) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/email/*.java` (3 files) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/oauth/*.java` (4 files incl. `OAuthProfile`) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/jwt/*.java` (2 files) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/scheduler/RefreshTokenPurgeScheduler.java` | New ← not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/security/*.java` (6 files incl. `RateLimitWebFilter`) | New |
| `services/gateway/src/main/java/io/cartogra/gateway/infrastructure/tracing/TraceContext.java` | New ← not in original plan |
| `services/gateway/src/main/java/io/cartogra/gateway/config/AppConfig.java` | New (JWT beans + OkHttp + WebClient + `@EnableScheduling`) |
| `services/gateway/src/main/java/io/cartogra/gateway/config/SecurityConfig.java` | Modified |
| `services/gateway/src/main/java/io/cartogra/gateway/config/RateLimitConfig.java` | New (Lua script bean — not SCG built-in) |
| `services/gateway/src/main/java/io/cartogra/gateway/config/RateLimitProperties.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/config/JwtConfig.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/config/ResendConfig.java` | New |
| `services/gateway/src/main/java/io/cartogra/gateway/config/OAuthConfig.java` | New |
| `services/gateway/src/test/java/io/cartogra/gateway/**/*.java` (12 test files) | New |
| `docs/diagrams/gateway/auth-schema-er.puml` | New (updated: includes `tenants` table) |
| `docs/diagrams/gateway/auth-domain-class.puml` | New (updated: includes `Tenant`, `TenantRepository`, `TraceContext`, `RefreshTokenPurgeScheduler`) |
...
| `docs/adr/ADR-0010-gateway-sole-token-issuer.md` | New (was planned as `ADR-001x`) |
| `docs/adr/ADR-0011-httponlycookie-bearer-dual-auth.md` | New (was planned as `ADR-001y`) |
