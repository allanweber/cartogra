...
- `ingestion.workers.k8s.enabled=false` for SCM-only test.

### How syncs are triggered

Syncs run via Kafka topic `cartogra.registry.sync.command`. There are two trigger paths:

1. **Auto-trigger on connection create** — `CreateScmConnectionUseCaseImpl` publishes a `cartogra.registry.sync.command` event immediately after saving a new SCM connection. This is implemented as part of task 1.55 (see Files Created / Modified table). No separate HTTP endpoint is needed.

2. **Re-trigger via Kafka UI** — open `http://localhost:8086` (Kafka UI), produce a message to `cartogra.registry.sync.command` with the JSON body shown below. Use this for smoke-testing an existing connection without re-creating it.
...
1. `POST /api/v1/scm-connections` with a valid GitHub org config — this auto-publishes `cartogra.registry.sync.command`.
2. Ensure the GitHub org has at least one non-archived repo containing `pom.xml` with a `spring-boot-starter` reference.
...
6. `GET /api/v1/services` with the tenant header; verify the discovered service appears with `techStack` and `lastCommitSha` set.
7. `GET /api/v1/services/{id}/history`; verify at least one history snapshot exists.
...
6. `GET /api/v1/services`; verify the K8s service row appears.

### Passing signal

- `GET /api/v1/services` returns the discovered service with `techStack` populated.
- `services_history` has a snapshot row.

### Failure triage

- No Kafka message on `service.discovered`: check ingestion logs for `ScmProviderException`; verify GitHub token and org config in the connection.
- No Kafka message on `sync.command`: check registry logs from `CreateScmConnectionUseCaseImpl`; verify producer is wired.
- Service not in registry: check registry logs for `RegistryServiceDiscoveryConsumer`; verify `external_id` unique constraint.
- Wrong techStack: check `TechStackDetector` logs; verify file-content responses from GitHub.
...
| `services/registry/src/main/java/io/cartogra/registry/application/usecase/impl/CreateScmConnectionUseCaseImpl.java` | Modify | Publish `cartogra.registry.sync.command` after saving; inject `SyncCommandProducer` |
| `services/registry/src/main/java/io/cartogra/registry/infrastructure/kafka/SyncCommandProducer.java` | New | Publishes `EventEnvelope<SyncCommandPayload>` to `cartogra.registry.sync.command` |
...
No new REST endpoints are introduced by this task. The SCM sync is triggered by creating a connection (`POST /api/v1/scm-connections`), which publishes `cartogra.registry.sync.command` via `CreateScmConnectionUseCaseImpl`. All discovery logic is internal (Kafka producers/consumers). Verified exclusively by integration tests (AC-8, AC-9).
...
6. Trigger a real sync by creating a new SCM connection (auto-publishes sync.command):
   POST {{gateway-url}}/api/v1/scm-connections
   Content-Type: application/json
   { "provider": "github", "config": "{\"token\":\"<pat>\",\"org\":\"<org>\"}" }
   # Expected: 201 Created — sync starts automatically via Kafka

7. Wait 15 s, then:
   GET {{gateway-url}}/api/v1/services?techStack=java
   # Expected: at least one service with techStack containing "java"

8. GET {{gateway-url}}/api/v1/services/{{discoveredServiceId}}/history
