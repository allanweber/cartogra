...
Cartogra is a **multi-tenant service intelligence platform**. It auto-discovers microservices from Kubernetes and SCM providers, maps declared and observed dependencies, guards API contracts, and surfaces AI-generated insights. All communication between services is either REST (synchronous) or Kafka (asynchronous); no service calls another service's database.
