---
mode: 'agent'
description: 'Scaffold a new Spring Boot microservice with hexagonal structure, Dockerfile, and initial Flyway migration'
---
...
**Usage:** provide the service name (e.g., `analytics`)

## Steps

1. **Determine package and paths**
   - Service name (kebab-case): user-supplied argument
...
2. **Create Gradle module** — `services/<name>/build.gradle.kts`:
   ...
   dependencies {
       ...
       // gRPC — include if the service exposes or calls internal gRPC APIs
...
3. **Create hexagonal package structure** under `src/main/java/io/cartogra/<name>/`:
   ...
   - `infrastructure/` — JDBC repositories, Kafka producers/consumers, gRPC clients and servers
   - `config/` — Spring beans, security config, OTel config

4. **Create `<Name>Application.java`**:
   ```java
   ...
   spring:
     ...
     threads:
       virtual:
         enabled: true
     datasource:
       url: jdbc:postgresql://${DB_HOST:localhost}:${DB_PORT:5432}/${DB_NAME:<name>_db}
...
7. **Create `GlobalExceptionHandler.java`** in `api/`:
   ```java
...
9. **Register the module** in root `settings.gradle.kts`:
...
10. **Verify rules checklist before finishing:**
    ...
    - [ ] Dockerfile uses multi-stage, non-root user (UID 1000), `MaxRAMPercentage=75`, no `latest` tag
    - [ ] First migration file exists
    - [ ] No `.proto` files inside this service — all contracts in `shared:contracts`
    - [ ] gRPC server impls in `infrastructure/grpc/`, annotated `@GrpcService`
    - [ ] gRPC clients in `infrastructure/grpc/`, inject channel via `@GrpcClient`
